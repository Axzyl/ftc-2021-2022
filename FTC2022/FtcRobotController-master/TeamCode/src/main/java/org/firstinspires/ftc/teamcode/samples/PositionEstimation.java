...
public class PositionEstimation {
    private static final double     COUNTS_PER_MOTOR_REV    = 537.7    ;// 435RPM-383.6, 1150RPM--145.6
    private static final double     DRIVE_GEAR_REDUCTION    = 1.0 ;     // the new one is 1:1, mecanum drive chassis
    private static final double     WHEEL_DIAMETER_M   = 0.096;//0.096;     // From gobilda mecanum drawingm should be 0.096
    ...
    private static final double DIS_2WHEELS = 0.296;          // for 4 mecanum wheels, actually test value is 0.415 distance between 2 wheels, for encoder turning
    private static final double DIS_L1 = DIS_2WHEELS /2;      // left-right wheels to center distance
    private static final double DIS_L2 = 0.120;                //for gobila, 7*24mm, 7 big hole distance,front-rear wheels to center distance
    ...
    private MecanumRobotDrive robot;
    ...
    public PositionEstimation(MecanumRobotDrive robot){
        this.robot = robot;
    ...
    private class EstimationThread extends Thread
    {
        ...
        public EstimationThread()
        {
            ...
            this.lfStartPos = robot.lf.getCurrentPosition();
            this.rfStartPos = robot.rf.getCurrentPosition();
            this.lrStartPos = robot.lr.getCurrentPosition();
            this.rrStartPos = robot.rr.getCurrentPosition();
        ...
        public void run()
        {
            ...
            try
            {
                while (!isInterrupted())
                {
                    ...
                    d1 = robot.lf.getCurrentPosition() - this.lfStartPos;
                    d1 = d1 / COUNTS_PER_M;
                    this.lfStartPos = robot.lf.getCurrentPosition();

                    d2 = robot.rf.getCurrentPosition() - this.rfStartPos;
                    d2 = d2 / COUNTS_PER_M;
                    this.rfStartPos = robot.rf.getCurrentPosition();

                    d3 = robot.lr.getCurrentPosition() - this.lrStartPos;
                    d3 = d3 / COUNTS_PER_M;
                    this.lrStartPos = robot.lr.getCurrentPosition();

                    d4 = robot.rr.getCurrentPosition() - this.rrStartPos;
                    d4 = d4 / COUNTS_PER_M;
                    this.rrStartPos = robot.rr.getCurrentPosition();
